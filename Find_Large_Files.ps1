...
$btnFind.Size = New-Object System.Drawing.Size(150,40)
$btnFind.Text = "Find Large Files"
$btnFind.Add_Click({
    $listBox.Items.Clear()
    $folder = $txtPath.Text
    ...
    $files = Get-ChildItem -Path $folder -Recurse -File -ErrorAction SilentlyContinue | Sort-Object Length -Descending | Select-Object -First 20
    foreach ($file in $files) {
        $sizeMB = "{0:N2}" -f ($file.Length / 1MB)
        $listBox.Items.Add("$($file.FullName) — $sizeMB MB")
    }
    if ($files.Count -eq 0) {
